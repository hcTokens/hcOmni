...
bool CMPTxList::LoadFreezeState(int blockHeight)
{
    ...
    PrintToConsole("Loading freeze state from levelDB\n");
    for (it->SeekToFirst(); it->Valid(); it->Next()) {
        PrintToConsole("Loading freeze state from levelDB 111111111111\n");
        std::string itData = it->value().ToString();
        std::vector<std::string> vstr;
        boost::split(vstr, itData, boost::is_any_of(":"), boost::token_compress_on);
        PrintToConsole("Loading freeze state from levelDB xxxxxxxxxxxx 555555555 \n");
        if (4 != vstr.size()) continue;
        PrintToConsole("Loading freeze state from levelDB xxxxxxxxxxxx 555555555 666666666\n");
        uint16_t txtype = atoi(vstr[2]);
        PrintToConsole("Loading freeze state from levelDB xxxxxxxxxxxx \n");
        ...
        PrintToConsole("Loading freeze state from levelDB xxxxxxxxxxxx 1111111111111111\n");
        if (atoi(vstr[0]) != 1) continue; // invalid, ignore
        uint256 txid = uint256S(it->key().ToString());
        PrintToConsole("Loading freeze state from levelDB 111111111111 2222222222222\n");
        int txPosition = p_OmniTXDB->FetchTransactionPosition(txid);
        PrintToConsole("Loading freeze state from levelDB 111111111111 33333333333\n");
        std::string sortKey = strprintf("%06d%010d", atoi(vstr[1]), txPosition);
        loadOrder.push_back(std::make_pair(sortKey, txid));
    }
    PrintToConsole("Loading freeze state from levelDB xxxxxxxxxxxx 555555555 9999999999999\n");
    delete it;
    ...
    for (std::vector<std::pair<std::string, uint256> >::iterator it = loadOrder.begin(); it != loadOrder.end(); ++it) {
        ...
        CTransaction wtx;
        CMPTransaction mp_obj;
        PrintToConsole("Loading freeze state from levelDB 22222222222\n");
        if (!GetTransaction(hash, wtx, Params().GetConsensus(), blockHash, true)) {
    ...
    PrintToConsole("Loading freeze state from levelDB xxxxxxxxxxxx 121212 \n");
