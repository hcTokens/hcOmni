...
{
    PrintToConsole("msc_initial_scan 11111111111\n");
...
int mastercore_init()
{
    ...
    PrintToConsole("Omni Core 000000000000000000000000000000\n");
	...
	PrintToConsole("Omni Core 77777777777777777777777777777777777\n");
    // legacy code, setting to pre-genesis-block
    int snapshotHeight = ConsensusParams().GENESIS_BLOCK - 1;

	PrintToConsole("Omni Core 88888888888888888888888888 \n");
    ...
    PrintToConsole("Omni Core 111111111111111111111111111\n");
    ...
    if (!p_txlistdb->LoadFreezeState(nWaterlineBlock)) {
        ...
        PrintToConsole(strShutdownReason);
        if (!GetBoolArg("-overrideforcedshutdown", false)) {
            AbortNode(strShutdownReason, strShutdownReason);
        }
    }
    PrintToConsole("Omni Core 5555555555555555555555555555555555555 %d\n", nWaterlineBlock);
    // initial scan
 //   msc_initial_scan(nWaterlineBlock);
    PrintToConsole("Omni Core 333333333333333333\n");
    // display Exodus balance
    int64_t exodus_balance = GetTokenBalance(exodus_address, OMNI_PROPERTY_MSC, BALANCE);
    PrintToLog("Exodus balance after initialization: %s\n", FormatDivisibleMP(exodus_balance));
    PrintToConsole("Omni Core 44444444444444444444444444\n");
