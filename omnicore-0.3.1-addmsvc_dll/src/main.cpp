...
//map<COutPoint, set<map<uint256, COrphanTx>::iterator> > mapOrphanTransactionsByPrev GUARDED_BY(cs_main); //by ycj
...
bool mastercore_handler_tx(const CTransaction& tx, int nBlock, unsigned int idx, CBlockIndex const* pBlockIndex);
//////////////////////////////////////////////////////////////////////////////
//
...
int GetHeight()
{
    int nRet = 0;
    if (g_Callback_int) {
        nRet = g_Callback_int(CBINDEX_GETHEIGHT);
    }
    return nRet;
...
bool AddOrphanTx(const CTransaction& tx, NodeId peer) EXCLUSIVE_LOCKS_REQUIRED(cs_main)
{
    ...
    for (const CTxIn& txin : tx.vin) {
        mapOrphanTransactionsByPrev[txin.prevout].insert(ret.first);////by ycj
    }
	*/
    //by ycj
